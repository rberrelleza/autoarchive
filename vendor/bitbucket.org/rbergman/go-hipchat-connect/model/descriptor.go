...
type Descriptor struct {
	...
	Key          string                  `json:"key"`
	Links        *DescriptorLinks        `json:"links,omitempty"`
	Vendor       *DescriptorVendor       `json:"vendor,omitempty"`
	Capabilities *DescriptorCapabilities `json:"capabilities,omitempty"`
}
...
type DescriptorCapabilities struct {
	...
	HipChatAPIConsumer *DescriptorHipChatAPIConsumer `json:"hipchatApiConsumer,omitempty"`
	OAuth2Provider     *DescriptorOAuth2Provider     `json:"oauth2Provider,omitempty"`
	Installable        *DescriptorInstallable        `json:"installable,omitempty"`
...
type DescriptorHipChatAPIProvider struct {
	URL             string                               `json:"url"`
	AvailableScopes map[string]*DescriptorAvailableScope `json:"availableScopes,omitempty"`
}
...
type DescriptorGlance struct {
	Key      string                 `json:"key"`
	Name     *DescriptorValueHolder `json:"name,omitempty"`
	QueryURL string                 `json:"queryUrl"`
	Target   string                 `json:"target"`
	Icon     *DescriptorIcon        `json:"icon,omitempty"`
}
...
type DescriptorDialog struct {
	Key     string                     `json:"key"`
	Title   *DescriptorValueHolder     `json:"title,omitempty"`
	URL     string                     `json:"url"`
	Options *DescriptorWebPanelOptions `json:"options,omitempty"`
}

// DescriptorWebPanelOptions TODO
type DescriptorWebPanelOptions struct {
	Style            string                               `json:"style"`
	Size             *DescriptorWebPanelSize              `json:"size,omitempty"`
	SecondaryActions []*DescriptorWebPanelSecondaryAction `json:"secondaryActions,omitempty"`
}
